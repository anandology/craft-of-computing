...
## To enable real-time collaboration, you must install the extension
#  `jupyter_collaboration`. You can install it using pip for example:
#  
...
## Whether custom CSS is loaded on the page. Defaults to False.
#  Default: False
...
## The extension manager factory to use. The default options are: "readonly" for
#  a manager without installation capability or "pypi" for a manager using
#  PyPi.org and pip to install extensions.
#  Default: 'pypi'
...
#  Default: '/home/anand/.jupyter/lab/user-settings'
# c.LabApp.user_settings_dir = '/home/anand/.jupyter/lab/user-settings'
...
#  Default: '/home/anand/.jupyter/lab/workspaces'
# c.LabApp.workspaces_dir = '/home/anand/.jupyter/lab/workspaces'
...
#  
#  When set to `True` (default in jupyter-server 2.0, subject to change in the
#  future), any request to an endpoint without an authentication rule (either
#  `@tornado.web.authenticated`, or `@allow_unauthenticated`) will be permitted,
...
#  `JUPYTER_SERVER_ALLOW_UNAUTHENTICATED_ACCESS` environment variable: any non-
#  empty value other than "true" and "yes" will prevent unauthenticated access to
#  endpoints without `@allow_unauthenticated`.
#  Default: True
...
## The random bytes used to secure cookies. By default this is generated on first
#  start of the server and persisted across server sessions by writing the cookie
#  secret into the `cookie_secret_file` file. When using an executable config
#  file you can override this to be random at each server restart.
#  
...
## If True, add a 'sandbox' directive to the Content-Security-Policy header for
#  nbconvert-served pages, confining any JavaScript to a unique origin so it
#  cannot interact with the Jupyter server.
#  Default: True
# c.ServerApp.nbconvert_csp_sandbox = True
...
c.FileContentsManager.preferred_dir = os.path.expanduser("~/python")
